<template>
  ...
  >
    ...
    <div>
      <div>
        <label for="text" class="block text-sm text-gray-800">E-mail</label>
        <input
          v-model="user.email"
          type="email"
          autocomplete="off"
      ...
      <div>
        <label for="text" class="block text-sm text-gray-800">Password</label>
        <input
...
export default {
  ...
  methods: {
    async userlogin() {
      ...
      const data = await this.$http.$post("http://localhost:5001/user/login", {
        body: this.user,
      });
      if (data.success) {
        if (this.role == "Student") {
          // this.$route.push(` ?type=${this.role}`);
          console.log("hii");
        } else {
          // this.$route.push(` ?type=${this.role}`);
          console.log("hii");
